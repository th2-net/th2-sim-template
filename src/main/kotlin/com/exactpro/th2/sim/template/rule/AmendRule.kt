...
class AmendRule(field: Map<String, Value>) : MessageCompareRule() {
    ...
    override fun handle(ruleContext: IRuleContext, incomeMessage: Message) {
        ...
        if (NOSRule.orders.containsKey(ordID) && NOSRule.orders[ordID] != null) {
            val fixAmend = NOSRule.orders[ordID]
            fixAmend!!.copyFields(
                incomeMessage,
                "Side",
                "Price",
            ...
            println(NOSRule.orders.keys.toString() + " amended " + ordID + ". Total:" + Companion.orders.size)
