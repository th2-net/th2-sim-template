...
class DemoScriptRule(field: Map<String, Value>) : MessageCompareRule() {
    ...
    private fun updateFile(record: String, newLine: Boolean = true) {
        ...
        if (csvFile.createNewFile()) {
            updateFile(
                "CsvRecordType,OrderQty,OrdType,SecurityIDSource,ClOrdID,OrderCapacity,AccountType,Side,Price,SecurityID,TransactTime,SecondaryClOrdID,OrderID,ExecID,LeavesQty,Text,ExecType,OrdStatus,CumQty",
                false
            )
        }
        try {
...
//                "48="+ message["SecurityID"] +"\u0001" +
    ...
    private fun withdrawMessage(incomeMessage: Message): MutableMap<String, String?> {
        ...
        for (field in listOf(
            ...
            toLog[field] =
                incomeMessage.getFieldsOrDefault(field, Value.newBuilder().setSimpleValue("").build())!!.getString()
        }
        for (field in listOf("BodyLength", "MsgSeqNum")) {
            toLog[field] = incomeMessage.getField("header")!!.getMessage()
                ?.getFieldsOrDefault(field, Value.newBuilder().setSimpleValue("").build())!!.getString()
        }
        toLog["SecurityID"] = incomeMessage.getField("Instrument")!!.getMessage()?.getField("SecurityID")!!.getString()
        toLog["SecurityIDSource"] =
            incomeMessage.getField("Instrument")!!.getMessage()?.getField("SecurityIDSource")!!.getString()
        toLog["SendingTime"] = timeFormatDirtyFix(
            incomeMessage.getField("header")!!.getMessage()
                ?.getFieldsOrDefault("SendingTime", Value.newBuilder().setSimpleValue("").build())!!.getString()
        )
        toLog["TransactTime"] = timeFormatDirtyFix(
            incomeMessage.getFieldsOrDefault(
                "TransactTime",
                Value.newBuilder().setSimpleValue("").build()
            )!!.getString()
        )
        toLog["CheckSum"] = incomeMessage.getField("trailer")!!.getMessage()?.getField("CheckSum")!!.getString()
        toLog["Sender"] = "DIRTY-CONN" + incomeMessage.metadata.id.connectionId.sessionAlias.last()
        return toLog
    }
                    ...
                    when (incomeMessage.getString("Side")) {
                        "1" -> {
                            ...
                            val fixNew = message("ExecutionReport")
                                .copyFields(
                                    incomeMessage,  // fields from NewOrder
                                    "Side",
                                    "Price",
                                    "CumQty",
                                    "ClOrdID",
                            ...
                            values = incomeMessage.getField("OrderQty")!!.getString() + "," +
                                    ...
                                    instrument.getString("SecurityIDSource") + "," +
                                    ...
                                    instrument.getString("SecurityID")
                            updateFile(
                                "D" + "," + values + "," +
                            ...
                            updateFile(
                                "8,$values,$transTime,,$ordId1,$execIdNew," +
                        ...
                        "2" -> {
                            ...
                            val repeating1 = message().addFields(
                                "NoPartyIDs", listOf(
                                    message().addFields(
                                        "PartyRole", "76",
                                        "PartyID", "DEMO-CONN1",
                            ...
                            val repeating2 = message().addFields(
                                "NoPartyIDs", listOf(
                                    message().addFields(
                                        "PartyRole", "76",
                                        "PartyID", "DEMO-CONN2",
